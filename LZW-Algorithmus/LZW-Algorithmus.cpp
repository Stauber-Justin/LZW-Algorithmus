...
static vector<vector<string>> LZW_Compress(const vector<char>& inputVector)
{
    ...
    // Falls der Eingabe-Vektor leer ist, sofort zur�ckgeben.
    ...
    for (size_t currentChar = 1; currentChar < inputVector.size(); currentChar++)
    {
        ...
        // �berpr�fen, ob currentCharString bereits im Dictionary enthalten ist.
        ...
        {
            // Nicht gefunden: F�ge currentCharString dem Dictionary hinzu.
            dictionary.push_back(currentCharString);
...
static string LZW_Decompress(const vector<vector<string>>& inputVector) {
    ...
    if (index >= 0 && index < outputDic.size()) {
        ...
        std::cerr << "Fehler: Index außerhalb des gültigen Bereichs!" << std::endl;
        return "";
    }
    ...
    for (int currentChar = 1; currentChar < outputCode.size(); currentChar++) {
        ...
        if (index >= 0 && index < outputDic.size()) {
            ...
            std::cerr << "Fehler: Index " << index << " außerhalb des gültigen Bereichs!" << std::endl;
            continue;
        }
    }
